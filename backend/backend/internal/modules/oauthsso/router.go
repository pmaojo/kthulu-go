...
import (
	...
	oauthhttp "github.com/pmaojo/kthulu-go/backend/internal/modules/oauthsso/adapters/http"
...
func NewRouter(provider fosite.OAuth2Provider, uc *usecase.OAuthUseCase) *Router {
	return &Router{
		handlers:   oauthhttp.NewHandlers(provider, uc),
		ssoHandler: oauthhttp.NewHandler(uc),
	}
}
...
func (r *Router) RegisterRoutes(root chi.Router) {
	...
	r.ssoHandler.RegisterRoutes(root)
}
