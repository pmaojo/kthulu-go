...
# If there's no ./frontend folder but there is a package.json in the parent
# (repo root), use that location as the frontend directory so script works
# when Docker and frontend are moved to repo root.
if [ ! -d "$FRONTEND_DIR" ] && [ -f "../package.json" ]; then
    FRONTEND_DIR=".."
fi
