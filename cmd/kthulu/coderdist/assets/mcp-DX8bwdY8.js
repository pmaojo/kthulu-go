...
import { AISDKError, ZodFirstPartyTypeKind, ZodIssueCode, z } from "./react-C-QzwZjq.js";
...
function _objectSpread(target) {
	for (var i$3 = 1; i$3 < arguments.length; i$3++) {
		var source = null != arguments[i$3] ? arguments[i$3] : {};
		i$3 % 2 ? ownKeys(Object(source), !0).forEach(function(key$1) {
...
function createDedent(options) {
	...
	function dedent$1(strings, ...values) {
		...
		let result = "";
		for (let i$3 = 0; i$3 < raw.length; i$3++) {
			let next = raw[i$3];
			if (escapeSpecialCharacters) next = next.replace(/\\\n[ \t]*/g, "").replace(/\\`/g, "`").replace(/\\\$/g, "$").replace(/\\\{/g, "{");
			result += next;
			if (i$3 < values.length) result += values[i$3];
		}
...
function parseUnionDef(def, refs) {
	...
	if (options.every((x) => x._def.typeName in primitiveMappings && (!x._def.checks || !x._def.checks.length))) {
		...
		if (types.length === options.length) {
			const uniqueTypes = types.filter((x, i$3, a) => a.indexOf(x) === i$3);
			return {
...
var asAnyOf = (def, refs) => {
	const anyOf = (def.options instanceof Map ? Array.from(def.options.values()) : def.options).map((x, i$3) => parseDef(x._def, {
		...refs,
		currentPath: [
			...refs.currentPath,
			"anyOf",
...
function parseTupleDef(def, refs) {
	if (def.rest) return {
		...
		items: def.items.map((x, i$3) => parseDef(x._def, {
			...refs,
			currentPath: [
				...refs.currentPath,
				"items",
...
function parseDef(def, refs, forceResolution = false) {
	...
	const jsonSchema$1 = selectParser(def, def.typeName, refs, forceResolution);
...
var get$ref = (item, refs) => {
	switch (refs.$refStrategy) {
		...
		case "extract-to-root":
			const name$2 = item.path.slice(refs.basePath.length + 1).join("_");
			if (name$2 !== refs.name && refs.nameStrategy === "duplicate-ref") refs.definitions[name$2] = item.def;
			return { $ref: [
				...refs.basePath,
				refs.definitionPath,
				name$2
...
var addMeta = (def, refs, jsonSchema$1) => {
	if (def.description) {
...
var zodToJsonSchema$1 = (schema, options) => {
	...
	const main$1 = parseDef(schema._def, name$2 === void 0 ? refs : {
		...refs,
		currentPath: [
			...refs.basePath,
			refs.definitionPath,
	...
	const definitions = (() => {
		if (isEmptyObj(refs.definitions)) return void 0;
		const definitions2 = {};
		const processedDefinitions = /* @__PURE__ */ new Set();
		for (let i$3 = 0; i$3 < 500; i$3++) {
			const newDefinitions = Object.entries(refs.definitions).filter(([key$1]) => !processedDefinitions.has(key$1));
			if (newDefinitions.length === 0) break;
			for (const [key$1, schema2] of newDefinitions) {
				definitions2[key$1] = parseDef(zodDef(schema2), {
					...refs,
					currentPath: [
						...refs.basePath,
						refs.definitionPath,
				...
				processedDefinitions.add(key$1);
			}
		}
		return definitions2;
	})();
	...
	} : main$1 : refs.nameStrategy === "duplicate-ref" ? {
		...main$1,
		...definitions || refs.seenRefs.size ? { [refs.definitionPath]: {
			...definitions,
			...refs.seenRefs.size ? { [name$2]: main$1 } : void 0
		} } : void 0
	} : {
		$ref: [
...
//#endregion
//#region node_modules/@agentic/core/dist/index.js
var __create$2 = Object.create;
var __defProp$2 = Object.defineProperty;
var __getOwnPropDesc$2 = Object.getOwnPropertyDescriptor;
var __knownSymbol$2 = (name$2, symbol$1) => (symbol$1 = Symbol[name$2]) ? symbol$1 : Symbol.for("Symbol." + name$2);
var __typeError$2 = (msg) => {
	throw TypeError(msg);
};
var __defNormalProp$2 = (obj, key$1, value) => key$1 in obj ? __defProp$2(obj, key$1, {
	enumerable: true,
	configurable: true,
	writable: true,
	value
...
var __decoratorContext$2 = (kind, name$2, done, metadata, fns) => ({
...
var __runInitializers$2 = (array, flags, self, value) => {
	for (var i$3 = 0, fns = array[flags >> 1], n = fns && fns.length; i$3 < n; i$3++) flags & 1 ? fns[i$3].call(self) : value = fns[i$3].call(self, value);
	return value;
};
var __decorateElement$2 = (array, flags, name$2, decorators, target, extra) => {
	var fn, it, done, ctx, access, k = flags & 7, s = !!(flags & 8), p = !!(flags & 16);
	var j = k > 3 ? array.length + 1 : k ? s ? 1 : 2 : 0, key$1 = __decoratorStrings$2[k + 5];
	...
	k ? p && k < 4 && __name$2(extra, (k > 2 ? "set " : k > 1 ? "get " : "") + name$2) : __name$2(target, name$2);
	for (var i$3 = decorators.length - 1; i$3 >= 0; i$3--) {
		ctx = __decoratorContext$2(k, name$2, done = {}, array[3], extraInitializers);
		if (k) {
			ctx.static = s, ctx.private = p, access = ctx.access = { has: p ? (x) => __privateIn$2(target, x) : (x) => name$2 in x };
			if (k ^ 3) access.get = p ? (x) => (k ^ 1 ? __privateGet$2 : __privateMethod$2)(x, target, k ^ 4 ? extra : desc.get) : (x) => x[name$2];
			if (k > 2) access.set = p ? (x, y) => __privateSet$2(x, target, y, k ^ 4 ? extra : desc.set) : (x, y) => x[name$2] = y;
		}
		it = (0, decorators[i$3])(k ? k < 4 ? p ? extra : desc[key$1] : k > 4 ? void 0 : {
			get: desc.get,
			set: desc.set
		} : target, ctx), done._ = 1;
		if (k ^ 4 || it === void 0) __expectFn$2(it) && (k > 4 ? initializers.unshift(it) : k ? p ? extra = it : desc[key$1] = it : target = it);
	...
	return k || __decoratorMetadata$2(array, target), desc && __defProp$2(target, name$2, desc), p ? k ^ 4 ? extra : desc : target;
};
...
function parseStructuredOutput(value, outputSchema) {
	if (!value || typeof value !== "string") throw new Error("Invalid output: expected string");
	const output = value;
	let result;
	if (outputSchema instanceof z.ZodArray || "element" in outputSchema) result = parseArrayOutput(output);
	else if (outputSchema instanceof z.ZodObject || "omit" in outputSchema) result = parseObjectOutput(output);
	else if (outputSchema instanceof z.ZodBoolean) result = parseBooleanOutput(output);
	else if (outputSchema instanceof z.ZodNumber || "nonnegative" in outputSchema) result = parseNumberOutput(output, outputSchema);
	else result = output;
	const safeResult = outputSchema.safeParse(result);
	if (!safeResult.success) throw fromZodError(safeResult.error);
	return safeResult.data;
}
function isEscaped(str, i$3) {
	return i$3 > 0 && str[i$3 - 1] === "\\" && !(i$3 > 1 && str[i$3 - 2] === "\\");
}
function extractJSONFromString(input, jsonStructureType) {
	...
	let nestingLevel = 0;
	let startIndex = 0;
	const isInsideQuoted = {
		"\"": false,
		"'": false
	};
	for (let i$3 = 0; i$3 < input.length; i$3++) {
		const ch = input.charAt(i$3);
		switch (ch) {
			case "\"":
			case "'":
				if (!isInsideQuoted[ch === "\"" ? "'" : "\""] && !isEscaped(input, i$3)) isInsideQuoted[ch] = !isInsideQuoted[ch];
				break;
			default: if (!isInsideQuoted["\""] && !isInsideQuoted["'"]) switch (ch) {
				case startChar:
					if (nestingLevel === 0) startIndex = i$3;
					nestingLevel += 1;
					break;
				case endChar:
					nestingLevel -= 1;
					if (nestingLevel === 0) {
						const candidate = input.slice(startIndex, i$3 + 1);
						const parsed = JSON.parse(jsonrepair(candidate));
						if (parsed && typeof parsed === "object") extractedJSONValues.push(parsed);
					} else if (nestingLevel < 0) throw new ParseError(`Invalid JSON string: unexpected ${endChar} at position ${i$3}`);
			}
		}
	}
	if (nestingLevel !== 0) throw new ParseError("Invalid JSON string: unmatched " + startChar + " or " + endChar);
...
function parseArrayOutput(output) {
	try {
		...
		if (arrayOutput.length === 0) throw new ParseError("Invalid JSON array");
		const parsedOutput = arrayOutput[0];
		if (!Array.isArray(parsedOutput)) throw new ParseError("Expected JSON array");
		return parsedOutput;
	} catch (err) {
		if (err instanceof JSONRepairError) throw new ParseError(err.message, { cause: err });
		else if (err instanceof SyntaxError) throw new ParseError(`Invalid JSON array: ${err.message}`, { cause: err });
		else throw err;
	}
}
function parseObjectOutput(output) {
	try {
		...
		if (arrayOutput.length === 0) throw new ParseError("Invalid JSON object");
		let parsedOutput = arrayOutput[0];
		if (Array.isArray(parsedOutput)) parsedOutput = parsedOutput[0];
		if (!parsedOutput || typeof parsedOutput !== "object") throw new ParseError("Expected JSON object");
		return parsedOutput;
	} catch (err) {
		if (err instanceof JSONRepairError) throw new ParseError(err.message, { cause: err });
		else if (err instanceof SyntaxError) throw new ParseError(`Invalid JSON object: ${err.message}`, { cause: err });
		else throw err;
	}
}
function parseBooleanOutput(output) {
	...
	if (booleanOutput === void 0) throw new ParseError(`Invalid boolean output: ${output}`);
	else return booleanOutput;
}
function parseNumberOutput(output, outputSchema) {
	...
	if (Number.isNaN(numberOutput)) throw new ParseError(`Invalid number output: ${output}`);
	return numberOutput;
}
function assert(value, message) {
	if (value) return;
	if (!message) throw new Error("Assertion failed");
...
function getEnv(name$2) {
	try {
		return typeof process !== "undefined" ? process.env?.[name$2] : void 0;
	} catch {
		return void 0;
	}
}
function sanitizeSearchParams(searchParams, { csv = false } = {}) {
	const entries = Object.entries(searchParams).flatMap(([key$1, value]) => {
		if (key$1 === void 0 || value === void 0) return [];
		if (Array.isArray(value)) return value.map((v) => [key$1, String(v)]);
		return [[key$1, String(value)]];
	});
	if (!csv) return new URLSearchParams(entries);
	const csvEntries = {};
	for (const [key$1, value] of entries) csvEntries[key$1] = csvEntries[key$1] ? `${csvEntries[key$1]},${value}` : value;
	return new URLSearchParams(csvEntries);
}
function stringifyForModel(jsonObject, replacer = null, space = 0) {
	if (jsonObject === void 0) return "";
	if (typeof jsonObject === "string") return jsonObject;
	return JSON.stringify(jsonObject, replacer, space);
}
var dedenter = dedent_default.withOptions({ escapeSpecialCharacters: true });
...
function zodToJsonSchema(schema, { strict: strict$1 = false } = {}) {
	return omit(zodToJsonSchema$1(schema, {
		$refStrategy: "none",
		openaiStrictMode: strict$1
...
function createJsonSchema(jsonSchema$1, { parse = (value) => value, safeParse, source } = {}) {
	safeParse ??= (value) => {
		try {
...
function createAIFunction({ name: name$2, description: description$1, inputSchema, strict: strict$1 = true, tags: tags$1, execute }, executeArg) {
	assert(name$2, "createAIFunction missing required \"name\"");
	assert(inputSchema, "createAIFunction missing required \"inputSchema\"");
	assert(execute || executeArg, "createAIFunction missing required \"execute\" function implementation");
	assert(!(execute && executeArg), "createAIFunction: cannot provide both \"execute\" and a second function argument. there should only be one function implementation.");
	execute ??= executeArg;
	assert(execute, "createAIFunction missing required \"execute\" function implementation");
	assert(typeof execute === "function", "createAIFunction \"execute\" must be a function");
	const inputAgenticSchema = asAgenticSchema(inputSchema, { strict: strict$1 });
	const parseInput = (input) => {
		if (typeof input === "string") return inputAgenticSchema.parse(input);
		else {
			const args = input.function_call?.arguments;
			assert(args, `Missing required function_call.arguments for function ${name$2}`);
...
if (typeof Symbol === "function" && Symbol.metadata) Object.defineProperty(globalThis, Symbol.metadata, {
	enumerable: true,
	configurable: true,
	writable: true,
	value: _metadata
...
var AIFunctionsProvider = class {
	...
	*/
	get functions() {
		if (!this._functions) {
			...
			assert(metadata, "Your runtime does not appear to support ES decorator metadata: https://github.com/tc39/proposal-decorator-metadata/issues/14");
	...
	*/
	getFunctionsFilteredByTags(...tags$1) {
		return this.functions.getFilteredByTags(...tags$1);
	}
	/**
...
function aiFunction(args) {
	return (_targetMethod, context) => {
		...
		if (!context.metadata.invocables) context.metadata.invocables = [];
...
var AIFunctionSet = class _AIFunctionSet {
	...
	constructor(aiFunctionLikeObjects, { transformNameKeysFn = transformName } = {}) {
		...
		const fns = aiFunctionLikeObjects?.flatMap((fn) => {
			if (fn instanceof AIFunctionsProvider) return [...fn.functions];
			if (fn instanceof _AIFunctionSet) return [...fn];
			if (isAIFunction(fn)) return fn;
			const fa = fn.functions ?? fn;
			if (fa) try {
				const fns2 = [...fa];
				if (fns2.every(isAIFunction)) return fns2;
			} catch {}
			throw new Error(`Invalid AIFunctionLike: ${fn}`);
		});
		if (fns) {
			for (const fn of fns) if (!isAIFunction(fn)) throw new Error(`Invalid AIFunctionLike: ${fn}`);
		}
		this._map = new Map(fns ? fns.map((fn) => [this._transformNameKeysFn(fn.spec.name), fn]) : null);
	}
	get size() {
	...
	get(name$2) {
		return this._map.get(this._transformNameKeysFn(name$2));
	}
	set(name$2, fn) {
		this._map.set(this._transformNameKeysFn(name$2), fn);
		return this;
	}
	has(name$2) {
		return this._map.has(this._transformNameKeysFn(name$2));
	}
	clear() {
		this._map.clear();
	}
	delete(name$2) {
		return this._map.delete(this._transformNameKeysFn(name$2));
	}
	pick(...keys) {
		const keysToIncludeSet = new Set(keys.map(this._transformNameKeysFn));
		return new _AIFunctionSet(Array.from(this).filter((fn) => keysToIncludeSet.has(this._transformNameKeysFn(fn.spec.name))));
	}
	omit(...keys) {
		const keysToExcludeSet = new Set(keys.map(this._transformNameKeysFn));
		return new _AIFunctionSet(Array.from(this).filter((fn) => !keysToExcludeSet.has(this._transformNameKeysFn(fn.spec.name))));
	}
	map(fn) {
	...
	*/
	getFilteredByTags(...tags$1) {
		const tagsSet = new Set(tags$1);
...
function transformName(name$2) {
	return name$2.toLowerCase();
}
var _echo_dec, _a$2, _init$2;
var EchoAITool = class extends (_a$2 = AIFunctionsProvider, _echo_dec = [aiFunction({
	name: "echo",
	description: "Echoes the input.",
	inputSchema: z.object({ query: z.string().describe("input query to echo") })
})], _a$2) {
	constructor() {
		super(...arguments);
...
__decorateElement$2(_init$2, 1, "echo", _echo_dec, EchoAITool);
__decoratorMetadata$2(_init$2, EchoAITool);
...
((Msg2) => {
	function system(content, opts) {
		const { name: name$2, cleanContent = true } = opts ?? {};
		return {
			role: "system",
	...
	function refusal(refusal2, opts) {
		const { name: name$2, cleanRefusal = true } = opts ?? {};
		return {
			role: "assistant",
	...
	function funcResult(content, name$2) {
	...
	function narrowResponseMessage(msg) {
		if (msg.content === null && msg.tool_calls != null) return Msg2.toolCall(msg.tool_calls);
		else if (msg.content === null && msg.function_call != null) return Msg2.funcCall(msg.function_call);
		else if (msg.content !== null && msg.content !== void 0) return Msg2.assistant(msg.content);
		else if (msg.refusal != null) return Msg2.refusal(msg.refusal);
		else {
	...
	function narrow(message) {
		...
		if (isDeveloper(message)) return message;
		if (isUser(message)) return message;
		if (isAssistant(message)) return message;
		if (isRefusal(message)) return message;
		if (isFuncCall(message)) return message;
		if (isFuncResult(message)) return message;
		if (isToolCall(message)) return message;
...
//#endregion
//#region node_modules/@agentic/ai-sdk/dist/index.js
function createAISDKTools(...aiFunctionLikeTools) {
	const fns = new AIFunctionSet(aiFunctionLikeTools);
	return Object.fromEntries(fns.map((fn) => [fn.spec.name, tool({
...
const getBodySize = (body) => {
	...
	if (body instanceof FormData) {
		let size = 0;
		for (const [key$1, value] of body) {
			size += usualFormBoundarySize;
			size += new TextEncoder().encode(`Content-Disposition: form-data; name="${key$1}"`).length;
...
const mergeHeaders = (source1 = {}, source2 = {}) => {
	...
	for (const [key$1, value] of source.entries()) if (isHeadersInstance && value === "undefined" || value === void 0) result.delete(key$1);
	else result.set(key$1, value);
	return result;
};
...
const deepMerge = (...sources) => {
	...
	for (const source of sources) if (Array.isArray(source)) {
		...
		for (let [key$1, value] of Object.entries(source)) {
			if (isObject(value) && key$1 in returnValue) value = deepMerge(returnValue[key$1], value);
			returnValue = {
				...returnValue,
				[key$1]: value
			};
...
const findUnknownOptions = (request, options) => {
	...
	for (const key$1 in options) if (!(key$1 in requestOptionsRegistry) && !(key$1 in kyOptionKeys) && !(key$1 in request)) unknownOptions[key$1] = options[key$1];
...
var Ky = class Ky {
	static create(input, options) {
		...
		const isRetriableMethod = ky$1._options.retry.methods.includes(ky$1.request.method.toLowerCase());
		const result = isRetriableMethod ? ky$1._retry(function_) : function_();
	...
	async _fetch() {
		...
		if (this._options.timeout === false) return this._options.fetch(mainRequest, nonRequestOptions);
...
const createInstance = (defaults$1) => {
	...
	ky$1.extend = (newDefaults) => {
		if (typeof newDefaults === "function") newDefaults = newDefaults(defaults$1 ?? {});
		return createInstance(validateAndMerge(defaults$1, newDefaults));
	};
	ky$1.stop = stop;
	return ky$1;
};
...
//#endregion
//#region node_modules/@agentic/searxng/dist/index.js
var __create$1 = Object.create;
var __defProp$1 = Object.defineProperty;
var __getOwnPropDesc$1 = Object.getOwnPropertyDescriptor;
var __knownSymbol$1 = (name$2, symbol$1) => (symbol$1 = Symbol[name$2]) ? symbol$1 : Symbol.for("Symbol." + name$2);
var __typeError$1 = (msg) => {
	throw TypeError(msg);
};
var __defNormalProp$1 = (obj, key$1, value) => key$1 in obj ? __defProp$1(obj, key$1, {
	enumerable: true,
	configurable: true,
	writable: true,
	value
...
var __decoratorContext$1 = (kind, name$2, done, metadata, fns) => ({
...
var __decorateElement$1 = (array, flags, name$2, decorators, target, extra) => {
	var fn, it, done, ctx, access, k = flags & 7, s = !!(flags & 8), p = !!(flags & 16);
	var j = k > 3 ? array.length + 1 : k ? s ? 1 : 2 : 0, key$1 = __decoratorStrings$1[k + 5];
	var initializers = k > 3 && (array[j - 1] = []), extraInitializers = array[j] || (array[j] = []);
	...
	k ? p && k < 4 && __name$1(extra, (k > 2 ? "set " : k > 1 ? "get " : "") + name$2) : __name$1(target, name$2);
	for (var i$3 = decorators.length - 1; i$3 >= 0; i$3--) {
		ctx = __decoratorContext$1(k, name$2, done = {}, array[3], extraInitializers);
		if (k) {
			ctx.static = s, ctx.private = p, access = ctx.access = { has: p ? (x) => __privateIn$1(target, x) : (x) => name$2 in x };
			if (k ^ 3) access.get = p ? (x) => (k ^ 1 ? __privateGet$1 : __privateMethod$1)(x, target, k ^ 4 ? extra : desc.get) : (x) => x[name$2];
			if (k > 2) access.set = p ? (x, y) => __privateSet$1(x, target, y, k ^ 4 ? extra : desc.set) : (x, y) => x[name$2] = y;
		}
		it = (0, decorators[i$3])(k ? k < 4 ? p ? extra : desc[key$1] : k > 4 ? void 0 : {
			get: desc.get,
			set: desc.set
		} : target, ctx), done._ = 1;
		if (k ^ 4 || it === void 0) __expectFn$1(it) && (k > 4 ? initializers.unshift(it) : k ? p ? extra = it : desc[key$1] = it : target = it);
	...
	return k || __decoratorMetadata$1(array, target), desc && __defProp$1(target, name$2, desc), p ? k ^ 4 ? extra : desc : target;
};
var __publicField$1 = (obj, key$1, value) => __defNormalProp$1(obj, typeof key$1 !== "symbol" ? key$1 + "" : key$1, value);
...
var searxng;
((searxng2) => {
	searxng2.SearchCategorySchema = z.enum([
		"general",
		"images",
		"videos",
		"news",
	...
	searxng2.SearchEngineSchema = z.enum([
		"9gag",
		"annas archive",
		"apk mirror",
		"apple app store",
	...
	searxng2.SearchOptionsSchema = z.object({
		query: z.string().describe("search query"),
		categories: z.array(searxng2.SearchCategorySchema).optional().describe("narrows the search to only use search engines in specific categories"),
		engines: z.array(searxng2.SearchEngineSchema).optional().describe("narrows the search to only use specific search engines"),
		language: z.string().optional(),
		pageno: z.number().int().optional()
	});
})(searxng || (searxng = {}));
var _search_dec$1, _a$1, _init$1;
var SearxngClient = class extends (_a$1 = AIFunctionsProvider, _search_dec$1 = [aiFunction({
	name: "searxng",
...
`,
	...
	constructor({ apiBaseUrl = getEnv("SEARXNG_API_BASE_URL"), ky: ky$1 = distribution_default } = {}) {
		assert(apiBaseUrl, "SearxngClient missing required \"apiBaseUrl\" (defaults to \"SEARXNG_API_BASE_URL\")");
		super();
		__runInitializers$1(_init$1, 5, this);
		__publicField$1(this, "ky");
		__publicField$1(this, "apiBaseUrl");
		this.apiBaseUrl = apiBaseUrl;
		this.ky = ky$1.extend({ prefixUrl: apiBaseUrl });
	}
	async search({ query,...opts }) {
		const res = await this.ky.get("search", { searchParams: pruneUndefined({
			...opts,
			q: query,
		...
		res.results = res.results?.map((result) => omit(result, "parsed_url", "engines", "positions", "template"));
		return pick(res, "results", "suggestions", "query");
	}
};
_init$1 = __decoratorStart$1(_a$1);
__decorateElement$1(_init$1, 1, "search", _search_dec$1, SearxngClient);
__decoratorMetadata$1(_init$1, SearxngClient);

//#endregion
//#region node_modules/@agentic/brave-search/dist/index.js
var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __knownSymbol = (name$2, symbol$1) => (symbol$1 = Symbol[name$2]) ? symbol$1 : Symbol.for("Symbol." + name$2);
var __typeError = (msg) => {
	throw TypeError(msg);
};
...
var __decoratorContext = (kind, name$2, done, metadata, fns) => ({
...
var __decorateElement = (array, flags, name$2, decorators, target, extra) => {
	var fn, it, done, ctx, access, k = flags & 7, s = !!(flags & 8), p = !!(flags & 16);
	var j = k > 3 ? array.length + 1 : k ? s ? 1 : 2 : 0, key$1 = __decoratorStrings[k + 5];
	var initializers = k > 3 && (array[j - 1] = []), extraInitializers = array[j] || (array[j] = []);
	...
	k ? p && k < 4 && __name(extra, (k > 2 ? "set " : k > 1 ? "get " : "") + name$2) : __name(target, name$2);
	for (var i$3 = decorators.length - 1; i$3 >= 0; i$3--) {
		ctx = __decoratorContext(k, name$2, done = {}, array[3], extraInitializers);
		if (k) {
			ctx.static = s, ctx.private = p, access = ctx.access = { has: p ? (x) => __privateIn(target, x) : (x) => name$2 in x };
			if (k ^ 3) access.get = p ? (x) => (k ^ 1 ? __privateGet : __privateMethod)(x, target, k ^ 4 ? extra : desc.get) : (x) => x[name$2];
			if (k > 2) access.set = p ? (x, y) => __privateSet(x, target, y, k ^ 4 ? extra : desc.set) : (x, y) => x[name$2] = y;
		}
		it = (0, decorators[i$3])(k ? k < 4 ? p ? extra : desc[key$1] : k > 4 ? void 0 : {
			get: desc.get,
			set: desc.set
		} : target, ctx), done._ = 1;
		if (k ^ 4 || it === void 0) __expectFn(it) && (k > 4 ? initializers.unshift(it) : k ? p ? extra = it : desc[key$1] = it : target = it);
		else if (typeof it !== "object" || it === null) __typeError("Object expected");
		else __expectFn(fn = it.get) && (desc.get = fn), __expectFn(fn = it.set) && (desc.set = fn), __expectFn(fn = it.init) && initializers.unshift(fn);
	}
	return k || __decoratorMetadata(array, target), desc && __defProp(target, name$2, desc), p ? k ^ 4 ? extra : desc : target;
};
...
var bravesearch;
((bravesearch2) => {
	bravesearch2.apiBaseUrl = "https://api.search.brave.com";
	bravesearch2.SearchParamsSchema = z.object({
		query: z.string().describe("Search query (max 400 chars, 50 words)").optional(),
		count: z.number().int().min(1).max(20).describe("Number of results (1-20, default 10)").optional(),
		offset: z.number().describe("Pagination offset (max 9, default 0)").optional()
	});
	bravesearch2.LocalSearchParamsSchema = z.object({
		query: z.string().describe("Search query (max 400 chars, 50 words)").optional(),
		count: z.number().describe("Number of results (1-20, default 10)").int().min(1).max(20).optional()
	});
})(bravesearch || (bravesearch = {}));
var _localSearch_dec, _search_dec, _a, _init;
var BraveSearchClient = class extends (_a = AIFunctionsProvider, _search_dec = [aiFunction({
	name: "brave_search",
	description: "Performs a web search using the Brave Search API, ideal for general queries, news, articles, and online content. Use this for broad information gathering, recent events, or when you need diverse web sources. Supports pagination, content filtering, and freshness controls. Maximum 20 results per request, with offset for pagination. ",
	inputSchema: bravesearch.SearchParamsSchema
})], _localSearch_dec = [aiFunction({
	...
	constructor({ apiKey = getEnv("BRAVE_SEARCH_API_KEY"), apiBaseUrl = bravesearch.apiBaseUrl, ky: ky$1 = distribution_default } = {}) {
		assert(apiKey, "BraveSearchClient missing required \"apiKey\" (defaults to \"BRAVE_SEARCH_API_KEY\")");
		super();
	...
	async search(queryOrParams) {
		const { query: q,...params } = typeof queryOrParams === "string" ? { query: queryOrParams } : queryOrParams;
		return this.ky.get("res/v1/web/search", { searchParams: sanitizeSearchParams({
			...params,
			q
		}) }).json();
	}
	async localSearch(queryOrParams) {
		const { query: q,...params } = typeof queryOrParams === "string" ? { query: queryOrParams } : queryOrParams;
		const webData = await this.ky.get("res/v1/web/search", { searchParams: sanitizeSearchParams({
			search_lang: "en",
			result_filter: "locations",
			...params,
			q
		}) }).json();
		const locationIds = webData.locations?.results?.filter((r) => !!r.id).map((r) => r.id);
		if (!locationIds?.length) return this.search(queryOrParams);
		const [pois, descriptions] = await Promise.all([this.getPoisData(locationIds), this.getDescriptionsData(locationIds)]);
		const desc = descriptions.descriptions;
		return Object.entries(desc).map(([id, description$1]) => ({
			description: description$1,
			...pois.results.find((r) => r.id === id)
		}));
	}
	async getPoisData(ids) {
		return this.ky.get("res/v1/local/pois", { searchParams: sanitizeSearchParams({ ids: ids.filter(Boolean) }) }).json();
	}
	async getDescriptionsData(ids) {
		return this.ky.get("res/v1/local/descriptions", { searchParams: sanitizeSearchParams({ ids: ids.filter(Boolean) }) }).json();
	}
};
_init = __decoratorStart(_a);
__decorateElement(_init, 1, "search", _search_dec, BraveSearchClient);
__decorateElement(_init, 1, "localSearch", _localSearch_dec, BraveSearchClient);
__decoratorMetadata(_init, BraveSearchClient);

//#endregion
//#region node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/named-references.js
var require_named_references = __commonJS({ "node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/named-references.js"(exports) {
	var __assign$1 = exports && exports.__assign || function() {
		__assign$1 = Object.assign || function(t) {
			for (var s, i$3 = 1, n = arguments.length; i$3 < n; i$3++) {
				s = arguments[i$3];
	...
	function generateNamedReferences(input, prev) {
		...
		for (var i$3 = 0; blocks.length > i$3; i$3++) {
			var entries = blocks[i$3].split(pairDivider);
...
//#endregion
//#region node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/numeric-unicode-map.js
var require_numeric_unicode_map = __commonJS({ "node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/numeric-unicode-map.js"(exports) {
	Object.defineProperty(exports, "__esModule", { value: true });
...
//#endregion
//#region node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/surrogate-pairs.js
var require_surrogate_pairs = __commonJS({ "node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/surrogate-pairs.js"(exports) {
	Object.defineProperty(exports, "__esModule", { value: true });
...
//#endregion
//#region node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/index.js
var require_commonjs = __commonJS({ "node_modules/duck-duck-scrape/node_modules/html-entities/dist/commonjs/index.js"(exports) {
	...
	function encode(text, _a$4) {
		var _b = _a$4 === void 0 ? defaultEncodeOptions : _a$4, _c = _b.mode, mode = _c === void 0 ? "specialChars" : _c, _d = _b.numeric, numeric = _d === void 0 ? "decimal" : _d, _e = _b.level, level = _e === void 0 ? "all" : _e;
		if (!text) return "";
		var encodeRegExp = encodeRegExps[mode];
		var references = allNamedReferences[level].characters;
		var isHex$1 = numeric === "hexadecimal";
		return String.prototype.replace.call(text, encodeRegExp, function(input) {
			var result = references[input];
			if (!result) {
				var code = input.length > 1 ? (0, surrogate_pairs_js_1.getCodePoint)(input, 0) : input.charCodeAt(0);
				result = (isHex$1 ? "&#x" + code.toString(16) : "&#" + code) + ";";
			}
			return result;
		});
	}
	...
	function decodeEntity(entity, _a$4) {
		var _b = _a$4 === void 0 ? defaultDecodeEntityOptions : _a$4, _c = _b.level, level = _c === void 0 ? "all" : _c;
		if (!entity) return "";
		return getDecodedEntity(entity, allNamedReferences[level].entities, false, false);
	}
	/** Decodes all entities in the text */
	function decode(text, _a$4) {
		var _b = _a$4 === void 0 ? defaultDecodeOptions : _a$4, _c = _b.level, level = _c === void 0 ? "all" : _c, _d = _b.scope, scope = _d === void 0 ? level === "xml" ? "strict" : "body" : _d;
		if (!text) return "";
...
//#endregion
//#region node_modules/needle/lib/querystring.js
var require_querystring = __commonJS({ "node_modules/needle/lib/querystring.js"(exports) {
	var toString = Object.prototype.toString;
	function stringify$1(obj, prefix) {
	...
	function stringifyArray(arr, prefix) {
		var ret = [];
		for (var i$3 = 0, len = arr.length; i$3 < len; i$3++) if (prefix) ret.push(stringify$1(arr[i$3], prefix + "[]"));
		else ret.push(stringify$1(arr[i$3]));
		return ret.join("&");
	}
	function stringifyObject(obj, prefix) {
		var ret = [];
		Object.keys(obj).forEach(function(key$1) {
			ret.push(stringify$1(obj[key$1], prefix ? prefix + "[" + encodeURIComponent(key$1) + "]" : encodeURIComponent(key$1)));
		});
		return ret.join("&");
	}
	exports.build = stringify$1;
} });

//#endregion
//#region node_modules/needle/lib/multipart.js
var require_multipart = __commonJS({ "node_modules/needle/lib/multipart.js"(exports) {
	...
	exports.build = function(data, boundary, callback) {
		...
		for (var key$1 in object) {
			...
			if (value === null || typeof value == "undefined") done();
				...
				generate_part(key$1, part, boundary, done);
			} else {
	...
	function generate_part(name$2, part, boundary, callback) {
		var return_part = "--" + boundary + "\r\n";
		return_part += "Content-Disposition: form-data; name=\"" + name$2 + "\"";
		function append(data, filename$1) {
			if (data) {
				var binary = part.content_type.indexOf("text") == -1;
				return_part += "; filename=\"" + encodeURIComponent(filename$1) + "\"\r\n";
		...
		if ((part.file || part.buffer) && part.content_type) {
			var filename = part.filename ? part.filename : part.file ? basename(part.file) : name$2;
			...
			if (typeof part.value == "object") return callback(new Error("Object received for " + name$2 + ", expected string."));
	...
	function flatten(object, into, prefix) {
		into = into || {};
		for (var key$1 in object) {
			var prefix_key = prefix ? prefix + "[" + key$1 + "]" : key$1;
			var prop = object[key$1];
...
//#endregion
//#region node_modules/needle/lib/auth.js
var require_auth = __commonJS({ "node_modules/needle/lib/auth.js"(exports, module) {
	...
	digest.parse_header = function(header) {
		...
		for (var i$3 = 0, l = matches.length; i$3 < l; i$3++) {
			var parts = matches[i$3].split("="), key$1 = parts.shift(), val = parts.join("=").replace(/^"/, "").replace(/"$/, "");
			challenge[key$1] = val;
		}
		return challenge;
	};
...
//#endregion
//#region node_modules/needle/lib/cookies.js
var require_cookies = __commonJS({ "node_modules/needle/lib/cookies.js"(exports) {
	...
	function writeCookieString(obj) {
		return Object.keys(obj).reduce(function(str, name$2) {
			var encodedName = encodeCookieComponent(name$2);
			var encodedValue = encodeCookieComponent(obj[name$2]);
...
var require_sax = __commonJS({ "node_modules/sax/lib/sax.js"(exports) {
	(function(sax$1) {
		sax$1.parser = function(strict$1, opt) {
		...
		function SAXParser(strict$1, opt) {
			...
			if (parser.opt.unquotedAttributeValues === void 0) parser.opt.unquotedAttributeValues = !strict$1;
		...
		if (!Object.keys) Object.keys = function(o) {
			var a = [];
			for (var i$3 in o) if (o.hasOwnProperty(i$3)) a.push(i$3);
			return a;
		};
		function checkBufferLength(parser) {
			var maxAllowed = Math.max(sax$1.MAX_BUFFER_LENGTH, 10);
			var maxActual = 0;
			for (var i$3 = 0, l = buffers.length; i$3 < l; i$3++) {
				var len = parser[buffers[i$3]].length;
				if (len > maxAllowed) switch (buffers[i$3]) {
					case "textNode":
						closeText(parser);
						break;
					case "cdata":
					...
					case "script":
						...
						parser.script = "";
						break;
					default: error(parser, "Max buffer length exceeded: " + buffers[i$3]);
				}
		...
		function clearBuffers(parser) {
			for (var i$3 = 0, l = buffers.length; i$3 < l; i$3++) parser[buffers[i$3]] = "";
		}
		...
		function isMatch(regex$1, c) {
			return regex$1.test(c);
		}
		function notMatch(regex$1, c) {
			return !isMatch(regex$1, c);
		}
		var S = 0;
		sax$1.STATE = {
			BEGIN: S++,
		...
		Object.keys(sax$1.ENTITIES).forEach(function(key$1) {
			var e = sax$1.ENTITIES[key$1];
			var s$1 = typeof e === "number" ? String.fromCharCode(e) : e;
			sax$1.ENTITIES[key$1] = s$1;
		});
		...
		function qname(name$2, attribute$1) {
			var i$3 = name$2.indexOf(":");
			var qualName = i$3 < 0 ? ["", name$2] : name$2.split(":");
			var prefix = qualName[0];
			var local = qualName[1];
			if (attribute$1 && name$2 === "xmlns") {
				prefix = "xmlns";
				local = "";
			}
			return {
		...
		function openTag(parser, selfClosing) {
			if (parser.opt.xmlns) {
				...
				for (var i$3 = 0, l = parser.attribList.length; i$3 < l; i$3++) {
					var nv = parser.attribList[i$3];
					var name$2 = nv[0];
					var value = nv[1];
					var qualName = qname(name$2, true);
					...
					parser.tag.attributes[name$2] = a;
		...
		function closeTag(parser) {
			...
			while (s$1-- > t) {
				...
				var x = {};
				for (var i$3 in tag.ns) x[i$3] = tag.ns[i$3];
				var parent = parser.tags[parser.tags.length - 1] || parser;
		...
		function charAt(chunk, i$3) {
			var result = "";
			if (i$3 < chunk.length) result = chunk.charAt(i$3);
			return result;
		}
				...
				switch (parser.state) {
					...
					case S.TEXT:
						if (parser.sawRoot && !parser.closedRoot) {
							...
							parser.textNode += chunk.substring(starti, i$3 - 1);
						}
...
//#endregion
//#region node_modules/needle/lib/parsers.js
var require_parsers = __commonJS({ "node_modules/needle/lib/parsers.js"(exports, module) {
	...
	function parseXML(str, cb) {
		...
		function newElement(name$2, attributes) {
			return {
				name: name$2 || "",
				value: "",
	...
	function parserFactory(name$2, fn) {
		function parser() {
			var chunks = [], stream$3 = new Transform({ objectMode: true });
			stream$3._transform = function(chunk, encoding, done) {
				chunks.push(chunk);
				done();
			};
			stream$3._flush = function(done) {
	...
	var parsers$1 = {};
	function buildParser(name$2, types, fn) {
		var parser = parserFactory(name$2, fn);
	...
	], function(buffer$1, cb) {
		var err, data;
		try {
			data = JSON.parse(buffer$1);
		} catch (e) {
			err = e;
		}
		cb(err, data);
	...
	buildParser("xml", [
		...
		parseXML(buffer$1.toString(), function(err, obj) {
			cb(err, obj);
		});
	});
	module.exports = parsers$1;
...
var require_bom_handling = __commonJS({ "node_modules/iconv-lite/lib/bom-handling.js"(exports) {
	var BOMChar = "﻿";
	...
	function StripBOMWrapper(decoder$1, options) {
...
var require_internal = __commonJS({ "node_modules/iconv-lite/encodings/internal.js"(exports, module) {
...
var require_utf32 = __commonJS({ "node_modules/iconv-lite/encodings/utf32.js"(exports) {
...
var require_utf16 = __commonJS({ "node_modules/iconv-lite/encodings/utf16.js"(exports) {
...
var require_utf7 = __commonJS({ "node_modules/iconv-lite/encodings/utf7.js"(exports) {
...
var require_sbcs_codec = __commonJS({ "node_modules/iconv-lite/encodings/sbcs-codec.js"(exports) {
...
var require_sbcs_data = __commonJS({ "node_modules/iconv-lite/encodings/sbcs-data.js"(exports, module) {
	module.exports = {
		"10029": "maccenteuro",
		"maccenteuro": {
			"type": "_sbcs",
...
var require_sbcs_data_generated = __commonJS({ "node_modules/iconv-lite/encodings/sbcs-data-generated.js"(exports, module) {
	module.exports = {
		"437": "cp437",
		"737": "cp737",
		"775": "cp775",
...
var require_dbcs_codec = __commonJS({ "node_modules/iconv-lite/encodings/dbcs-codec.js"(exports) {
	...
	DBCSDecoder.prototype.write = function(buf) {
		...
		for (var i$3 = 0, j = 0; i$3 < buf.length; i$3++) {
			...
			if (uCode >= 0) {} else if (uCode === UNASSIGNED) {
				...
				if (i$3 >= 3) {
					var ptr = (buf[i$3 - 3] - 129) * 12600 + (buf[i$3 - 2] - 48) * 1260 + (buf[i$3 - 1] - 129) * 10 + (curByte - 48);
				} else {
					var ptr = (prevBytes[i$3 - 3 + prevOffset] - 129) * 12600 + ((i$3 - 2 >= 0 ? buf[i$3 - 2] : prevBytes[i$3 - 2 + prevOffset]) - 48) * 1260 + ((i$3 - 1 >= 0 ? buf[i$3 - 1] : prevBytes[i$3 - 1 + prevOffset]) - 129) * 10 + (curByte - 48);
				}
...
var init_shiftjis = __esm({ "node_modules/iconv-lite/encodings/tables/shiftjis.json"() {
...
var eucjp_default;
var init_eucjp = __esm({ "node_modules/iconv-lite/encodings/tables/eucjp.json"() {
...
var cp936_default;
var init_cp936 = __esm({ "node_modules/iconv-lite/encodings/tables/cp936.json"() {
...
var init_gbk_added = __esm({ "node_modules/iconv-lite/encodings/tables/gbk-added.json"() {
	gbk_added_default = [
		[
			"a140",
			"",
...
var init_gb18030_ranges = __esm({ "node_modules/iconv-lite/encodings/tables/gb18030-ranges.json"() {
	uChars = [
		128,
		165,
		169,
...
var cp949_default;
var init_cp949 = __esm({ "node_modules/iconv-lite/encodings/tables/cp949.json"() {
...
var cp950_default;
var init_cp950 = __esm({ "node_modules/iconv-lite/encodings/tables/cp950.json"() {
...
var init_big5_added = __esm({ "node_modules/iconv-lite/encodings/tables/big5-added.json"() {
...
var require_dbcs_data = __commonJS({ "node_modules/iconv-lite/encodings/dbcs-data.js"(exports, module) {
	module.exports = {
		"shiftjis": {
			type: "_dbcs",
			table: function() {
				return init_shiftjis(), __toCommonJS(shiftjis_exports);
			},
			encodeAdd: {
				"¥": 92,
				"‾": 126
		...
		"eucjp": {
			type: "_dbcs",
			table: function() {
				return init_eucjp(), __toCommonJS(eucjp_exports);
			},
			encodeAdd: {
				"¥": 92,
				"‾": 126
		...
		"cp936": {
			type: "_dbcs",
			table: function() {
				return init_cp936(), __toCommonJS(cp936_exports);
			}
		},
		"gbk": {
			type: "_dbcs",
			table: function() {
				return (init_cp936(), __toCommonJS(cp936_exports)).concat((init_gbk_added(), __toCommonJS(gbk_added_exports)));
			}
		},
		"xgbk": "gbk",
		"isoir58": "gbk",
		"gb18030": {
			...
			gb18030: function() {
				return init_gb18030_ranges(), __toCommonJS(gb18030_ranges_exports);
			},
			encodeSkipVals: [128],
			encodeAdd: { "€": 41699 }
		},
		...
		"cp949": {
			type: "_dbcs",
			table: function() {
				return init_cp949(), __toCommonJS(cp949_exports);
			}
		},
		"cseuckr": "cp949",
		"csksc56011987": "cp949",
		...
		"cp950": {
			type: "_dbcs",
			table: function() {
				return init_cp950(), __toCommonJS(cp950_exports);
			}
		},
		"big5": "big5hkscs",
		"big5hkscs": {
			type: "_dbcs",
			table: function() {
				return (init_cp950(), __toCommonJS(cp950_exports)).concat((init_big5_added(), __toCommonJS(big5_added_exports)));
			},
			encodeSkipVals: [
				36457,
				36463,
...
var require_encodings = __commonJS({ "node_modules/iconv-lite/encodings/index.js"(exports) {
	var modules = [
		require_internal(),
		require_utf32(),
		require_utf16(),
...
var require_streams = __commonJS({ "node_modules/iconv-lite/lib/streams.js"(exports, module) {
	var Buffer$2 = require_safer().Buffer;
	module.exports = function(stream_module$1) {
		var Transform$1 = stream_module$1.Transform;
		...
		IconvLiteDecoderStream.prototype._transform = function(chunk, encoding, done) {
			if (!Buffer$2.isBuffer(chunk) && !(chunk instanceof Uint8Array)) return done(new Error("Iconv decoding stream needs buffers as its input."));
			try {
				var res = this.conv.write(chunk);
...
var require_lib$1 = __commonJS({ "node_modules/iconv-lite/lib/index.js"(exports, module) {
	...
	iconv$1.encode = function encode$1(str, encoding, options) {
		str = "" + (str || "");
	...
	iconv$1.decode = function decode$1(buf, encoding, options) {
		...
		var decoder$1 = iconv$1.getDecoder(encoding, options);
		var res = decoder$1.write(buf);
		var trail = decoder$1.end();
	...
	iconv$1.getDecoder = function getDecoder(encoding, options) {
		var codec = iconv$1.getCodec(encoding), decoder$1 = new codec.decoder(options, codec);
		if (codec.bomAware && !(options && options.stripBOM === false)) decoder$1 = new bomHandling.StripBOM(decoder$1, options);
		return decoder$1;
	};
...
//#endregion
//#region node_modules/needle/lib/decoder.js
var require_decoder = __commonJS({ "node_modules/needle/lib/decoder.js"(exports, module) {
	var iconv, inherits = __require("util").inherits, stream$2 = __require("stream");
	var regex = /(?:charset|encoding)\s*=\s*['"]? *([\w\-]+)/i;
	inherits(StreamDecoder, stream$2.Transform);
	function StreamDecoder(charset) {
		if (!(this instanceof StreamDecoder)) return new StreamDecoder(charset);
		stream$2.Transform.call(this, charset);
	...
	module.exports = function(charset) {
		try {
			if (!iconv) iconv = require_lib$1();
		} catch (e) {}
		if (iconv) return new StreamDecoder(charset);
		else return new stream$2.PassThrough();
	};
} });

//#endregion
//#region node_modules/needle/lib/utils.js
var require_utils = __commonJS({ "node_modules/needle/lib/utils.js"(exports, module) {
	var fs$1 = __require("fs"), url$1 = __require("url"), stream$1 = __require("stream");
	function resolve_url(href, base) {
		if (url$1.URL) return new url$1.URL(href, base);
		return base ? url$1.resolve(base, href) : href;
	}
	function host_and_ports_match(url1, url2) {
		if (url1.indexOf("http") < 0) url1 = "http://" + url1;
		if (url2.indexOf("http") < 0) url2 = "http://" + url2;
		var a = url$1.parse(url1), b = url$1.parse(url2);
	...
	function should_proxy_to(uri) {
		...
		for (var i$3 in pattern_list) {
			pattern = pattern_list[i$3];
			if (pattern.trim().length == 0) continue;
			var regex$1 = new RegExp(pattern.replace(/^\./, "*").replace(/[.]/g, "\\$&").replace(/\*/g, ".*"));
			if (uri.match(regex$1)) return false;
		}
		return true;
	}
	function get_env_var(keys, try_lower) {
		var val, i$3 = -1, env = process.env;
		while (!val && i$3 < keys.length - 1) {
			val = env[keys[++i$3]];
			if (!val && try_lower) val = env[keys[i$3].toLowerCase()];
		}
		return val;
	}
	...
	function get_stream_length(stream$3, given_length, cb) {
		if (given_length > 0) return cb(given_length);
		if (stream$3.end !== void 0 && stream$3.end !== Infinity && stream$3.start !== void 0) return cb(stream$3.end + 1 - (stream$3.start || 0));
		fs$1.stat(stream$3.path, function(err, stat) {
			cb(stat ? stat.size - (stream$3.start || 0) : null);
		});
	}
	function pump_streams(streams, cb) {
		if (stream$1.pipeline) return stream$1.pipeline.apply(null, streams.concat(cb));
...
//#endregion
//#region node_modules/needle/package.json
var package_exports = {};
...
var name, version$1, description, keywords, tags, author, repository, dependencies, devDependencies, scripts, directories, main, bin, license, engines, package_default;
var init_package = __esm({ "node_modules/needle/package.json"() {
	name = "needle";
	version$1 = "3.3.1";
...
var require_needle = __commonJS({ "node_modules/needle/lib/needle.js"(exports, module) {
	var fs = __require("fs"), http = __require("http"), https = __require("https"), url = __require("url"), stream = __require("stream"), debug = __require("util").debuglog("needle"), stringify = require_querystring().build, multipart = require_multipart(), auth = require_auth(), cookies = require_cookies(), parsers = require_parsers(), decoder = require_decoder(), utils = require_utils();
	var version = (init_package(), __toCommonJS(package_exports)).version;
	...
	Needle.prototype.setup = function(uri, options) {
		function get_option(key$2, fallback) {
			if (typeof options[key$2] != "undefined") return options[key$2];
			return typeof options[aliased.inverted[key$2]] != "undefined" ? options[aliased.inverted[key$2]] : fallback;
		}
		function check_value(expected, key$2) {
			var value = get_option(key$2), type = typeof value;
			if (type != "undefined" && type != expected) throw new TypeError(type + " received for " + key$2 + ", but expected a " + expected);
			return type == expected ? value : defaults[key$2];
		}
		var config = {
			http_opts: {
		...
		keys_by_type(Boolean).forEach(function(key$2) {
			config[key$2] = check_value("boolean", key$2);
		});
		keys_by_type(Number).forEach(function(key$2) {
			config[key$2] = check_value("number", key$2);
		});
		...
		tls_options.split(" ").forEach(function(key$2) {
			if (typeof options[key$2] != "undefined") if (config.http_opts.agent) config.http_opts.agent.options[key$2] = options[key$2];
			else config.http_opts[key$2] = options[key$2];
		});
		for (var key$1 in defaults.headers) config.headers[key$1] = defaults.headers[key$1];
		...
		if (config.proxy) if (!config.use_proxy_from_env_var || utils.should_proxy_to(uri)) {
	...
	Needle.prototype.start = function() {
		...
		function next(body$1) {
			if (body$1) {
				if (body$1.length) config.headers["content-length"] = body$1.length;
			...
			self.send_request(1, method, uri, config, body$1, out, callback);
		}
	...
	Needle.prototype.send_request = function(count, method, uri, config, post_data, out, callback) {
		...
		request = protocol.request(request_opts, function(resp) {
			...
			if (redirect_codes.indexOf(resp.statusCode) !== -1 && self.should_follow(headers.location, config, uri)) {
				...
				} else if (config.follow_max > 0) return done(new Error("Max redirects reached. Possible loop in: " + headers.location));
			}
			...
			var pipeline = [], mime = utils.parse_content_type(headers["content-type"]), text_response = mime.type && (mime.type.indexOf("text/") != -1 || !!mime.type.match(/(\/|\+)(xml|json)$/));
		...
		if (post_data) if (utils.is_stream(post_data)) utils.pump_streams([post_data, request], function(err) {
			if (err) debug(err);
		});
		else {
		...
		if (signal) if (signal.aborted === true) abort_handler();
		else signal.addEventListener("abort", abort_handler, { once: true });
		out.abort = function() {
			request.destroy();
		};
		out.request = request;
	...
	module.exports.defaults = function(obj) {
		for (var key$1 in obj) {
			var target_key = aliased.options[key$1] || key$1;
			if (defaults.hasOwnProperty(target_key) && typeof obj[key$1] != "undefined") {
				if (target_key != "parse_response" && target_key != "proxy" && target_key != "agent" && target_key != "signal") {
					var valid_type = defaults[target_key].constructor.name;
					if (obj[key$1].constructor.name != valid_type) throw new TypeError("Invalid type for " + key$1 + ", should be " + valid_type);
				} else if (target_key === "signal" && obj[key$1] !== null && !(obj[key$1] instanceof AbortSignal)) throw new TypeError("Invalid type for " + key$1 + ", should be AbortSignal");
				defaults[target_key] = obj[key$1];
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/util.js
var require_util = __commonJS({ "node_modules/duck-duck-scrape/lib/util.js"(exports) {
	...
	const needle_1$17 = __importDefault$17(require_needle());
	/** @internal */
	...
	var SafeSearchType;
	(function(SafeSearchType$1) {
		/** Strict filtering, no NSFW content. */
		SafeSearchType$1[SafeSearchType$1["STRICT"] = 0] = "STRICT";
		/** Moderate filtering. */
		SafeSearchType$1[SafeSearchType$1["MODERATE"] = -1] = "MODERATE";
		/** No filtering. */
		SafeSearchType$1[SafeSearchType$1["OFF"] = -2] = "OFF";
	})(SafeSearchType = exports.SafeSearchType || (exports.SafeSearchType = {}));
	/** The type of time ranges of the search results in DuckDuckGo. */
	var SearchTimeType;
	(function(SearchTimeType$1) {
		/** From any time. */
		SearchTimeType$1["ALL"] = "a";
		/** From the past day. */
		SearchTimeType$1["DAY"] = "d";
		/** From the past week. */
		SearchTimeType$1["WEEK"] = "w";
		/** From the past month. */
		SearchTimeType$1["MONTH"] = "m";
		/** From the past year. */
		SearchTimeType$1["YEAR"] = "y";
	...
	*/
	async function getVQD(query, ia = "web", options) {
		try {
			const response = await (0, needle_1$17.default)("get", `https://duckduckgo.com/?${queryString({
				q: query,
				ia
			})}`, options);
	...
	function parseSpiceBody(body, regex$1 = /^ddg_spice_[\w]+\(\n?((?:.|\n)+)\n?\);?/) {
		return JSON.parse(regex$1.exec(body.toString())[1]);
	}
	exports.parseSpiceBody = parseSpiceBody;
} });

//#endregion
//#region node_modules/duck-duck-scrape/lib/search/images.js
var require_images = __commonJS({ "node_modules/duck-duck-scrape/lib/search/images.js"(exports) {
	...
	const html_entities_1$3 = require_commonjs();
	const needle_1$16 = __importDefault$16(require_needle());
	const util_1$17 = require_util();
	/** The types of image sizes. */
	var ImageSize;
	(function(ImageSize$1) {
		/** Any size. */
		ImageSize$1["ALL"] = "";
		/** Small size, less than 200x200. */
		ImageSize$1["SMALL"] = "Small";
		/** Medium size, approx. between 200x200 and 500x500. */
		ImageSize$1["MEDIUM"] = "Medium";
		/** Large size, approx. between 500x500 and 2000x2000. */
		ImageSize$1["LARGE"] = "Large";
		/** Wallpaper size, larger than 1200x1200. */
		ImageSize$1["WALLPAPER"] = "Wallpaper";
	})(ImageSize = exports.ImageSize || (exports.ImageSize = {}));
	/** The types of images. */
	var ImageType;
	(function(ImageType$1) {
		/** Any images. */
		ImageType$1["ALL"] = "";
		/** Any regular photos. */
		ImageType$1["PHOTOGRAPH"] = "photo";
		/** Clipart. */
		ImageType$1["CLIPART"] = "clipart";
		/** Animated GIFs. */
		ImageType$1["GIF"] = "gif";
		/** Transparent photos. */
		ImageType$1["TRANSPARENT"] = "transparent";
	})(ImageType = exports.ImageType || (exports.ImageType = {}));
	/** The types of image layouts. */
	var ImageLayout;
	(function(ImageLayout$1) {
		/** Any size of images. */
		ImageLayout$1["ALL"] = "";
		/** Square images. Images may not be exactly square. */
		ImageLayout$1["SQUARE"] = "Square";
		/** Tall images. More height than width. */
		ImageLayout$1["TALL"] = "Tall";
		/** Wide images. More width than height. */
		ImageLayout$1["WIDE"] = "Wide";
	})(ImageLayout = exports.ImageLayout || (exports.ImageLayout = {}));
	/** The types of image colors. */
	var ImageColor;
	(function(ImageColor$1) {
		/** Any image. */
		ImageColor$1["ALL"] = "";
		/** Any image with color. */
		ImageColor$1["COLOR"] = "color";
		/** Any monochome images. */
		ImageColor$1["BLACK_AND_WHITE"] = "Monochrome";
		/** Mostly red images. */
		ImageColor$1["RED"] = "Red";
		/** Mostly orange images. */
		ImageColor$1["ORANGE"] = "Orange";
		/** Mostly yellow images. */
		ImageColor$1["YELLOW"] = "Yellow";
		/** Mostly green images. */
		ImageColor$1["GREEN"] = "Green";
		/** Mostly blue images. */
		ImageColor$1["BLUE"] = "Blue";
		/** Mostly pink images. */
		ImageColor$1["PINK"] = "Pink";
		/** Mostly brown images. */
		ImageColor$1["BROWN"] = "Brown";
		/** Mostly black images. */
		ImageColor$1["BLACK"] = "Black";
		/** Mostly gray images. */
		ImageColor$1["GRAY"] = "Gray";
		/** Alias for `GRAY`. */
		ImageColor$1["GREY"] = "Gray";
		/** Mostly teal images. */
		ImageColor$1["TEAL"] = "Teal";
		/** Mostly white images. */
		ImageColor$1["WHITE"] = "White";
	})(ImageColor = exports.ImageColor || (exports.ImageColor = {}));
	/** The types of image licenses. */
	var ImageLicense;
	(function(ImageLicense$1) {
		/** Any image license. */
		ImageLicense$1["ALL"] = "";
		/** All Creative Commons. */
		ImageLicense$1["CREATIVE_COMMONS"] = "Any";
		/** Public Domain images. */
		ImageLicense$1["PUBLIC_DOMAIN"] = "Public";
		/** Free to share and use. */
		ImageLicense$1["SHARE"] = "Share";
		/** Free to share and use commercially. */
		ImageLicense$1["SHARE_COMMERCIALLY"] = "ShareCommercially";
		/** Free to modify, share, and use. */
		ImageLicense$1["MODIFY"] = "Modify";
		/** Free to modify, share, and use commercially. */
		ImageLicense$1["MODIFY_COMMERCIALLY"] = "ModifyCommercially";
	})(ImageLicense = exports.ImageLicense || (exports.ImageLicense = {}));
	const defaultOptions$3 = {
		safeSearch: util_1$17.SafeSearchType.OFF,
		locale: "en-us",
		offset: 0
	};
	/**
	...
	*/
	async function searchImages(query, options, needleOptions) {
		...
		if (!options) options = defaultOptions$3;
		else options = sanityCheck$3(options);
		let vqd = options.vqd;
		if (!vqd) vqd = await (0, util_1$17.getVQD)(query, "web", needleOptions);
		const filters = [
		...
		const response = await (0, needle_1$16.default)("get", `https://duckduckgo.com/i.js?${(0, util_1$17.queryString)(queryObject)}`, needleOptions || { headers: util_1$17.COMMON_HEADERS });
		if (response.statusCode === 403) throw new Error("A server error occurred!");
		const imagesResult = (0, util_1$17.ensureJSON)(response.body);
		return {
			noResults: !imagesResult.results.length,
			vqd,
			results: imagesResult.results.map((image) => ({
				...image,
				title: (0, html_entities_1$3.decode)(image.title)
			}))
		};
	}
	exports.searchImages = searchImages;
	function sanityCheck$3(options) {
		options = Object.assign({}, defaultOptions$3, options);
		if (!(options.safeSearch in util_1$17.SafeSearchType)) throw new TypeError(`${options.safeSearch} is an invalid safe search type!`);
		if (typeof options.safeSearch === "string") options.safeSearch = util_1$17.SafeSearchType[options.safeSearch];
		...
		if (options.layout && !Object.values(ImageLayout).includes(options.layout)) throw new TypeError(`${options.layout} is an invalid image layout filter!`);
		...
		if (options.license && !Object.values(ImageLicense).includes(options.license)) throw new TypeError(`${options.license} is an invalid license filter!`);
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/search/news.js
var require_news = __commonJS({ "node_modules/duck-duck-scrape/lib/search/news.js"(exports) {
	var __importDefault$15 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.searchNews = void 0;
	const html_entities_1$2 = require_commonjs();
	const needle_1$15 = __importDefault$15(require_needle());
	const util_1$16 = require_util();
	const defaultOptions$2 = {
		safeSearch: util_1$16.SafeSearchType.OFF,
		locale: "en-us",
		offset: 0
	};
	/**
	...
	*/
	async function searchNews(query, options, needleOptions) {
		if (!query) throw new Error("Query cannot be empty!");
		if (!options) options = defaultOptions$2;
		else options = sanityCheck$2(options);
		let vqd = options.vqd;
		if (!vqd) vqd = await (0, util_1$16.getVQD)(query, "web", needleOptions);
		const queryObject = {
			l: options.locale,
			o: "json",
			noamp: "1",
		...
		const response = await (0, needle_1$15.default)("get", `https://duckduckgo.com/news.js?${(0, util_1$16.queryString)(queryObject)}`, needleOptions || { headers: util_1$16.COMMON_HEADERS });
		if (response.statusCode === 403) throw new Error("A server error occurred!");
		const newsResult = (0, util_1$16.ensureJSON)(response.body);
		return {
			noResults: !newsResult.results.length,
			vqd,
			results: newsResult.results.map((article) => ({
				date: article.date,
				excerpt: (0, html_entities_1$2.decode)(article.excerpt),
				image: article.image,
				relativeTime: article.relative_time,
				syndicate: article.syndicate,
				title: (0, html_entities_1$2.decode)(article.title),
				url: article.url,
	...
	function sanityCheck$2(options) {
		options = Object.assign({}, defaultOptions$2, options);
		if (!(options.safeSearch in util_1$16.SafeSearchType)) throw new TypeError(`${options.safeSearch} is an invalid safe search type!`);
		if (typeof options.safeSearch === "string") options.safeSearch = util_1$16.SafeSearchType[options.safeSearch];
		if (typeof options.offset !== "number") throw new TypeError(`Search offset is not a number!`);
		if (options.offset < 0) throw new RangeError("Search offset cannot be below zero!");
		if (!options.locale || typeof options.locale !== "string") throw new TypeError("Search locale must be a string!");
		if (options.time && !Object.values(util_1$16.SearchTimeType).includes(options.time)) throw new TypeError(`${options.time} is an invalid time filter!`);
		if (options.vqd && !/\d-\d+-\d+/.test(options.vqd)) throw new Error(`${options.vqd} is an invalid VQD!`);
		return options;
	}
} });
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/search/search.js
var require_search = __commonJS({ "node_modules/duck-duck-scrape/lib/search/search.js"(exports) {
	var __importDefault$14 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.autocomplete = exports.search = void 0;
	const html_entities_1$1 = require_commonjs();
	const needle_1$14 = __importDefault$14(require_needle());
	const util_1$15 = require_util();
	const defaultOptions$1 = {
		safeSearch: util_1$15.SafeSearchType.OFF,
		time: util_1$15.SearchTimeType.ALL,
		locale: "en-us",
		region: "wt-wt",
		offset: 0,
		marketRegion: "en-US"
	...
	*/
	async function search(query, options, needleOptions) {
		if (!query) throw new Error("Query cannot be empty!");
		if (!options) options = defaultOptions$1;
		else options = sanityCheck$1(options);
		let vqd = options.vqd;
		if (!vqd) vqd = await (0, util_1$15.getVQD)(query, "web", needleOptions || { headers: util_1$15.COMMON_HEADERS });
		const queryObject = {
			q: query,
			...options.safeSearch !== util_1$15.SafeSearchType.STRICT ? { t: "D" } : {},
			l: options.locale,
			...options.safeSearch === util_1$15.SafeSearchType.STRICT ? { p: "1" } : {},
			...
			df: options.time,
			vqd,
			...options.safeSearch !== util_1$15.SafeSearchType.STRICT ? { ex: String(options.safeSearch) } : {},
			sp: "1",
			bpa: "1",
			biaexp: "b",
			msvrtexp: "b",
			...options.safeSearch === util_1$15.SafeSearchType.STRICT ? {
				videxp: "a",
				nadse: "b",
				eclsexp: "a",
				stiaexp: "a",
		...
		const response = await (0, needle_1$14.default)("get", `https://links.duckduckgo.com/d.js?${(0, util_1$15.queryString)(queryObject)}`, needleOptions);
		...
		if (searchResults.length === 1 && !("n" in searchResults[0])) {
			...
			if (!onlyResult.da && onlyResult.t === "EOF" || !onlyResult.a || onlyResult.d === "google.com search") return {
				noResults: true,
				vqd,
				results: []
			};
		...
		for (const search$1 of searchResults) {
			if ("n" in search$1) continue;
			let bang;
			if (search$1.b) {
				const [prefix, title, domain] = search$1.b.split("	");
				bang = {
					prefix,
					title,
					domain
			...
			results.results.push({
				title: search$1.t,
				description: (0, html_entities_1$1.decode)(search$1.a),
				rawDescription: search$1.a,
				hostname: search$1.i,
				icon: `https://external-content.duckduckgo.com/ip3/${search$1.i}.ico`,
				url: search$1.u,
				bang
			});
		}
		...
		if (imagesMatch) {
			...
			results.images = imagesResult.results.map((i$3) => {
				i$3.title = (0, html_entities_1$1.decode)(i$3.title);
				return i$3;
			});
		}
		...
		if (newsMatch) {
			...
			results.news = newsResult.results.map((article) => ({
				date: article.date,
				excerpt: (0, html_entities_1$1.decode)(article.excerpt),
				image: article.image,
				relativeTime: article.relative_time,
				syndicate: article.syndicate,
				title: (0, html_entities_1$1.decode)(article.title),
				url: article.url,
				isOld: !!article.is_old
			}));
		}
		...
		if (videosMatch) {
			...
			results.videos = [];
			for (const video of videoResult.results) results.videos.push({
				url: video.content,
				title: (0, html_entities_1$1.decode)(video.title),
				description: (0, html_entities_1$1.decode)(video.description),
		...
		if (relatedMatch) {
			...
			results.related = [];
			for (const related of relatedResult.results) results.related.push({
				text: related.text,
	...
	function sanityCheck$1(options) {
		options = Object.assign({}, defaultOptions$1, options);
		if (!(options.safeSearch in util_1$15.SafeSearchType)) throw new TypeError(`${options.safeSearch} is an invalid safe search type!`);
		if (typeof options.safeSearch === "string") options.safeSearch = util_1$15.SafeSearchType[options.safeSearch];
		if (typeof options.offset !== "number") throw new TypeError(`Search offset is not a number!`);
		if (options.offset < 0) throw new RangeError("Search offset cannot be below zero!");
		if (options.time && !Object.values(util_1$15.SearchTimeType).includes(options.time) && !/\d{4}-\d{2}-\d{2}..\d{4}-\d{2}-\d{2}/.test(options.time)) throw new TypeError(`${options.time} is an invalid search time!`);
		if (!options.locale || typeof options.locale !== "string") throw new TypeError("Search locale must be a string!");
	...
	*/
	async function autocomplete(query, region, needleOptions) {
		...
		const response = await (0, needle_1$14.default)("get", `https://duckduckgo.com/ac/?${(0, util_1$15.queryString)(queryObject)}`, needleOptions);
		return (0, util_1$15.ensureJSON)(response.body);
	}
	exports.autocomplete = autocomplete;
} });

//#endregion
//#region node_modules/duck-duck-scrape/lib/search/videos.js
var require_videos = __commonJS({ "node_modules/duck-duck-scrape/lib/search/videos.js"(exports) {
	...
	const html_entities_1 = require_commonjs();
	const needle_1$13 = __importDefault$13(require_needle());
	const util_1$14 = require_util();
	/** The types of video definition. */
	var VideoDefinition;
	(function(VideoDefinition$1) {
		/** Any definition. */
		VideoDefinition$1["ANY"] = "";
		/** High definition. */
		VideoDefinition$1["HIGH"] = "high";
		/** Standard definition. */
		VideoDefinition$1["STANDARD"] = "standard";
	})(VideoDefinition = exports.VideoDefinition || (exports.VideoDefinition = {}));
	/** The types of video duration. */
	var VideoDuration;
	(function(VideoDuration$1) {
		/** Any video duration. */
		VideoDuration$1["ANY"] = "";
		/** Short videos, shorter than ~5 minutes. */
		VideoDuration$1["SHORT"] = "short";
		/** Medium length videos, between 5 and 20 minutes. */
		VideoDuration$1["MEDIUM"] = "medium";
		/** Long videos, longer than 20 minutes. */
		VideoDuration$1["LONG"] = "long";
	})(VideoDuration = exports.VideoDuration || (exports.VideoDuration = {}));
	/** The types of video licenses. */
	var VideoLicense;
	(function(VideoLicense$1) {
		/** Any video license. */
		VideoLicense$1["ANY"] = "";
		/** Creative Commons license. */
		VideoLicense$1["CREATIVE_COMMONS"] = "creativeCommon";
		/** YouTube Standard license. */
		VideoLicense$1["YOUTUBE"] = "youtube";
	})(VideoLicense = exports.VideoLicense || (exports.VideoLicense = {}));
	const defaultOptions = {
		safeSearch: util_1$14.SafeSearchType.OFF,
		locale: "en-us",
		offset: 0
	};
	/**
	...
	*/
	async function searchVideos(query, options, needleOptions) {
		...
		let vqd = options.vqd;
		if (!vqd) vqd = await (0, util_1$14.getVQD)(query, "web", needleOptions);
		const filters = [
		...
		const response = await (0, needle_1$13.default)("get", `https://duckduckgo.com/v.js?${(0, util_1$14.queryString)(queryObject)}`, needleOptions || { headers: util_1$14.COMMON_HEADERS });
		if (response.statusCode === 403) throw new Error("A server error occurred!");
		const videosResult = (0, util_1$14.ensureJSON)(response.body);
		return {
	...
	function sanityCheck(options) {
		...
		if (!(options.safeSearch in util_1$14.SafeSearchType)) throw new TypeError(`${options.safeSearch} is an invalid safe search type!`);
		if (typeof options.safeSearch === "string") options.safeSearch = util_1$14.SafeSearchType[options.safeSearch];
		if (typeof options.offset !== "number") throw new TypeError(`Search offset is not a number!`);
		if (options.offset < 0) throw new RangeError("Search offset cannot be below zero!");
		if (!options.locale || typeof options.locale !== "string") throw new TypeError("Search locale must be a string!");
		if (options.time && !Object.values(util_1$14.SearchTimeType).includes(options.time)) throw new TypeError(`${options.time} is an invalid time filter!`);
		if (options.definition && !Object.values(VideoDefinition).includes(options.definition)) throw new TypeError(`${options.definition} is an invalid video definition!`);
		if (options.duration && !Object.values(VideoDuration).includes(options.duration)) throw new TypeError(`${options.duration} is an invalid video duration!`);
		if (options.license && !Object.values(VideoLicense).includes(options.license)) throw new TypeError(`${options.license} is an invalid video license!`);
		if (options.vqd && !/\d-\d+-\d+/.test(options.vqd)) throw new Error(`${options.vqd} is an invalid VQD!`);
		return options;
	}
} });
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/currency.js
var require_currency = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/currency.js"(exports) {
	var __importDefault$12 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.currency = void 0;
	const needle_1$12 = __importDefault$12(require_needle());
	...
	*/
	async function currency(from, to, amount = 1, needleOptions) {
		...
		const response = await (0, needle_1$12.default)("get", `${util_1$13.SPICE_BASE}/currency/${amount}/${from}/${to}`, needleOptions);
		const result = (0, util_1$13.parseSpiceBody)(response.body);
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/dictionary/audio.js
var require_audio = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/dictionary/audio.js"(exports) {
	var __importDefault$11 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.dictionaryAudio = void 0;
	const needle_1$11 = __importDefault$11(require_needle());
	...
	*/
	async function dictionaryAudio(word, needleOptions) {
		...
		const response = await (0, needle_1$11.default)("get", `${util_1$12.SPICE_BASE}/dictionary/audio/${word}`, needleOptions);
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/dictionary/definition.js
var require_definition = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/dictionary/definition.js"(exports) {
	var __importDefault$10 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.dictionaryDefinition = void 0;
	const needle_1$10 = __importDefault$10(require_needle());
	...
	*/
	async function dictionaryDefinition(word, needleOptions) {
		if (!word) throw new Error("Word cannot be empty!");
		const response = await (0, needle_1$10.default)("get", `${util_1$11.SPICE_BASE}/dictionary/definition/${word}`, needleOptions);
		if (response.body.toString() === "ddg_spice_dictionary_definition();\n") return [];
		const result = (0, util_1$11.parseSpiceBody)(response.body);
		return result;
	}
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/dictionary/hyphenation.js
var require_hyphenation = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/dictionary/hyphenation.js"(exports) {
	var __importDefault$9 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.dictionaryHyphenation = void 0;
	const needle_1$9 = __importDefault$9(require_needle());
	...
	*/
	async function dictionaryHyphenation(word, needleOptions) {
		if (!word) throw new Error("Word cannot be empty!");
		const response = await (0, needle_1$9.default)("get", `${util_1$10.SPICE_BASE}/dictionary/hyphenation/${word}`, needleOptions);
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/dictionary/pronunciation.js
var require_pronunciation = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/dictionary/pronunciation.js"(exports) {
	var __importDefault$8 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.dictionaryPronunciation = void 0;
	const needle_1$8 = __importDefault$8(require_needle());
	...
	*/
	async function dictionaryPronunciation(word, needleOptions) {
		if (!word) throw new Error("Word cannot be empty!");
		const response = await (0, needle_1$8.default)("get", `${util_1$9.SPICE_BASE}/dictionary/pronunciation/${word}`, needleOptions);
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/dns.js
var require_dns = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/dns.js"(exports) {
	var __importDefault$7 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.dns = exports.DNSRecordType = void 0;
	const needle_1$7 = __importDefault$7(require_needle());
	const util_1$8 = require_util();
	/** A type of DNS record. */
	var DNSRecordType;
	(function(DNSRecordType$1) {
		DNSRecordType$1["ANY"] = "any";
		DNSRecordType$1["A"] = "a";
		DNSRecordType$1["AAAA"] = "aaaa";
		DNSRecordType$1["AFSDB"] = "afsdb";
		DNSRecordType$1["APL"] = "apl";
		DNSRecordType$1["CAA"] = "caa";
		DNSRecordType$1["CERT"] = "cert";
		DNSRecordType$1["CNAME"] = "cname";
		DNSRecordType$1["DHCID"] = "dhcid";
		DNSRecordType$1["DLV"] = "dlv";
		DNSRecordType$1["DNAME"] = "dname";
		DNSRecordType$1["DNSKEY"] = "dnskey";
		DNSRecordType$1["DS"] = "ds";
		DNSRecordType$1["HIP"] = "hip";
		DNSRecordType$1["IPSECKEY"] = "ipseckey";
		DNSRecordType$1["KEY"] = "key";
		DNSRecordType$1["KX"] = "kx";
		DNSRecordType$1["LOC"] = "loc";
		DNSRecordType$1["MX"] = "mx";
		DNSRecordType$1["NS"] = "ns";
		DNSRecordType$1["NSEC"] = "nsec";
		DNSRecordType$1["NSEC3"] = "nsec3";
		DNSRecordType$1["NSEC3PARAM"] = "nsec3param";
		DNSRecordType$1["RRSIG"] = "rrsig";
		DNSRecordType$1["RP"] = "rp";
		DNSRecordType$1["SIG"] = "sig";
		DNSRecordType$1["SOA"] = "soa";
		DNSRecordType$1["SPF"] = "spf";
		DNSRecordType$1["SRV"] = "srv";
		DNSRecordType$1["SSHFP"] = "sshfp";
		DNSRecordType$1["TA"] = "ta";
		DNSRecordType$1["TKEY"] = "tkey";
		DNSRecordType$1["TLSA"] = "tlsa";
		DNSRecordType$1["TSIG"] = "tsig";
		DNSRecordType$1["TX"] = "tx";
		DNSRecordType$1["TXT"] = "txt";
	...
	*/
	async function dns(domain, recordType = DNSRecordType.ANY, needleOptions) {
		...
		const response = await (0, needle_1$7.default)("get", `${util_1$8.SPICE_BASE}/dns/${recordType}/${domain}`, needleOptions);
		const result = (0, util_1$8.parseSpiceBody)(response.body);
		return result;
	}
	exports.dns = dns;
} });
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/emojipedia.js
var require_emojipedia = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/emojipedia.js"(exports) {
	var __importDefault$6 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.emojipedia = void 0;
	const needle_1$6 = __importDefault$6(require_needle());
	...
	*/
	async function emojipedia(emoji, needleOptions) {
		...
		const response = await (0, needle_1$6.default)("get", `${util_1$7.SPICE_BASE}/emojipedia/${emoji}`, needleOptions);
		const result = (0, util_1$7.parseSpiceBody)(response.body);
		return result;
	}
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/expandUrl.js
var require_expandUrl = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/expandUrl.js"(exports) {
	var __importDefault$5 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.expandUrl = void 0;
	const needle_1$5 = __importDefault$5(require_needle());
	...
	*/
	async function expandUrl(url$2, needleOptions) {
		if (!url$2) throw new Error("URL cannot be empty!");
		const response = await (0, needle_1$5.default)("get", `${util_1$6.SPICE_BASE}/expand_url/${encodeURIComponent(url$2)}`, needleOptions);
		const result = (0, util_1$6.parseSpiceBody)(response.body);
		return result;
	}
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/forecast.js
var require_forecast = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/forecast.js"(exports) {
	var __importDefault$4 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.forecast = void 0;
	const needle_1$4 = __importDefault$4(require_needle());
	...
	*/
	async function forecast(query, locale = "en", needleOptions) {
		...
		const response = await (0, needle_1$4.default)("get", `${util_1$5.SPICE_BASE}/forecast/${encodeURIComponent(query)}/${locale}`, needleOptions);
		if (response.body.toString() === "ddg_spice_forecast();\n") return null;
		return (0, util_1$5.parseSpiceBody)(response.body, /ddg_spice_[\w]+\(\n?((?:.|\n)+)\n?\);?/);
	}
	exports.forecast = forecast;
} });

//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/statista.js
var require_statista = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/statista.js"(exports) {
	var __importDefault$3 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.statista = void 0;
	const needle_1$3 = __importDefault$3(require_needle());
	...
	*/
	async function statista(query, needleOptions) {
		const response = await (0, needle_1$3.default)("get", `${util_1$4.SPICE_BASE}/statista/${encodeURIComponent(query)}`, needleOptions);
		const result = (0, util_1$4.parseSpiceBody)(response.body);
		return result;
	}
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/stocks.js
var require_stocks = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/stocks.js"(exports) {
	var __importDefault$2 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.stocks = void 0;
	const needle_1$2 = __importDefault$2(require_needle());
	...
	*/
	async function stocks(symbol$1, needleOptions) {
		if (!symbol$1) throw new Error("Symbol cannot be empty!");
		const response = await (0, needle_1$2.default)("get", `${util_1$3.SPICE_BASE}/stocks/${symbol$1}`, needleOptions);
		const result = (0, util_1$3.parseSpiceBody)(response.body);
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/thesaurus.js
var require_thesaurus = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/thesaurus.js"(exports) {
	var __importDefault$1 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.thesaurus = void 0;
	const needle_1$1 = __importDefault$1(require_needle());
	...
	*/
	async function thesaurus(word, needleOptions) {
		if (!word) throw new Error("Word cannot be empty!");
		const response = await (0, needle_1$1.default)("get", `${util_1$2.SPICE_BASE}/thesaurus/${word}`, needleOptions);
		if (response.body.toString() === "ddg_spice_thesaurus();\n") return null;
		const result = (0, util_1$2.parseSpiceBody)(response.body);
		return result;
	}
...
//#endregion
//#region node_modules/duck-duck-scrape/lib/spices/time.js
var require_time = __commonJS({ "node_modules/duck-duck-scrape/lib/spices/time.js"(exports) {
	var __importDefault = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.time = void 0;
	const needle_1 = __importDefault(require_needle());
	...
	*/
	async function time(query, needleOptions) {
		if (!query) throw new Error("Query cannot be empty!");
		const response = await (0, needle_1.default)("get", `${util_1$1.SPICE_BASE}/time/${query}`, needleOptions);
		return (0, util_1$1.parseSpiceBody)(response.body);
	}
	exports.time = time;
} });

//#endregion
//#region node_modules/duck-duck-scrape/lib/index.js
var require_lib = __commonJS({ "node_modules/duck-duck-scrape/lib/index.js"(exports) {
	...
	__exportStar(require_images(), exports);
	__exportStar(require_news(), exports);
	__exportStar(require_search(), exports);
	__exportStar(require_videos(), exports);
	__exportStar(require_currency(), exports);
	__exportStar(require_audio(), exports);
	__exportStar(require_definition(), exports);
	__exportStar(require_hyphenation(), exports);
	__exportStar(require_pronunciation(), exports);
	__exportStar(require_dns(), exports);
	__exportStar(require_emojipedia(), exports);
	__exportStar(require_expandUrl(), exports);
	__exportStar(require_forecast(), exports);
	__exportStar(require_statista(), exports);
	__exportStar(require_stocks(), exports);
	__exportStar(require_thesaurus(), exports);
	__exportStar(require_time(), exports);
	var util_1 = require_util();
	Object.defineProperty(exports, "getVQD", {
		enumerable: true,
		get: function() {
...
var require_react_jsx_runtime_production = __commonJS({ "node_modules/react/cjs/react-jsx-runtime.production.js"(exports) {
	var REACT_ELEMENT_TYPE = Symbol.for("react.transitional.element"), REACT_FRAGMENT_TYPE = Symbol.for("react.fragment");
	function jsxProd(type, config, maybeKey) {
		var key$1 = null;
		void 0 !== maybeKey && (key$1 = "" + maybeKey);
		void 0 !== config.key && (key$1 = "" + config.key);
		if ("key" in config) {
			maybeKey = {};
...
//#endregion
//#region src/mcp.tsx
async function playwright({ executablePath, runner } = {}) {
...
async function webSearch({ provider } = {}) {
	if (provider === "duckduckgo" || !provider) return { search_web: {
		...
		async *generate({ query, maxResults }) {
			yield (0, import_jsx_runtime.jsx)(
				"span",
				// remove the parameter
				{ children: "Searching..." }
);
			const results = await (0, import_lib.search)(query, { safeSearch: import_lib.SafeSearchType.STRICT }, { uri_modifier: (rawUrl) => {
				const url$2 = new URL(rawUrl);
				url$2.searchParams.delete("ss_mkt");
				return url$2.toString();
			} });
			yield (0, import_jsx_runtime.jsxs)("span", { children: [
				"Found ",
				results.results.length,
				" results"
			] });
			yield `Here are the results: <search-results>${results.results.map((result) => {
	...
	if (provider === "brave") {
		...
		return { search_web: {
			description: "Performs a web search using the Brave Search API, ideal for general queries, news, articles, and online content. Use this for broad information gathering, recent events, or when you need diverse web sources. Maximum 20 results per request, with offset for pagination. ",
			parameters: z.object({
				query: z.string({ description: "Search query" }).min(1).max(128),
				maxResults: z.number().min(1).max(100).optional()
			}),
			async *generate({ query, maxResults }) {
				yield (0, import_jsx_runtime.jsx)("span", { children: "Searching..." });
				...
				yield `Here are the results: <search-results>${results.web?.results?.map((result) => {
					return `<search-result>
              <title>${result.title}</title>
              <description>${result.description}</description>
              <url>${result.url}</url>
...
async function createMcp({ name: name$2, command, args }) {
	const transport = new StdioMCPTransport({
		command,
		args
	});
...
//#endregion
export { createMcp, dedent_default, import_jsx_runtime, import_jsx_runtime$1, playwright, webSearch };