...
import (
        ...
        "github.com/example/workshop-api/internal/adapters/http/modules/cars/domain"
)
...
func (h *CarsHandler) RegisterRoutes(r *mux.Router) {
        r.HandleFunc("/carss", h.List).Methods("GET")
        r.HandleFunc("/carss", h.Create).Methods("POST")
        r.HandleFunc("/carss/{id}", h.GetByID).Methods("GET")
}
